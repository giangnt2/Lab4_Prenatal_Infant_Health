...
**Note from Carmen: I do not think this is what the assignment is asking us to do. We are making a lot of assumptions here and I don't support all of them (for example, the idea that a healthy mother makes fewer prenatal visits -- a mother who makes 0 prenatal visits is probably poor or unhealthy or lacks access to healthcare). Our week 14 async videos clearly define causality in terms of the model error, and so we should discuss that here.**
In model1, model2 and model3, we  pointed out that the assumptions CLM1, CLM2, CLM3 are met. In all the models, npvis was also proved to be statistically significant while monpre is not significant so we now just investigate npvis.
To further check the causality of npvis to baby weight, we test if the npvis is not correlated with the error term 
```{r}
cor(v$npvis, model1$residuals)
cor(v$npvis, model2$residuals)
cor(v$npvis, model3$residuals)
```
The numbers all come out to very near zero. This means that we can assume $d(bwght)/d(npvis) =beta_{npvis}$ as $d(u)/d(npvis) = 0 $
So we can say that npvis does have positive causal effect to baby's weight (all npvis coefficients are positive).
## Direction of biases

...
+ CML6 (Normality of errors): The distribution of residuals plot shows that residual distribution is quite close to the normal distribution shape. Furthermore, the Q-Q plot shows that our error follows the normal distribution quite closely. Therefore we can assume that CLM6 is met for model 1.
...
```{r}
model2 <- lm(bwght ~ monpre + npvis + male + cigs + drink + mage + magesq + meduc + mwhte + mblck, data = v)
...
+ An extra cigarette per day is associated with a 10 gram (0.35 ounces) reduction in birthweight. This is not practically significant, since a mother would need to smoke an additional 46 cigarettes *every day of her pregnancy* before we would expect to see a 1-pound reduction in weight.
...
We are concerned that the coefficient on `monpre` is positive, which would indicate that babies benefit (in terms of higher birthweight) from starting prenatal care *later* in the pregnancy. This relationship requires further study.
...
+ CML6 (Normality): The distribution of residuals plot shows that the residuals are quite close to the normal distribution shape. Furthermore, the Q-Q plot shows that our error follows the normal distribution quite closely. Therefore we can assume that CLM6 is met for model 3.
...
In Model 1, Model 2, and Model 3, we pointed out that the assumptions CLM1, CLM2, CLM3 are met. In each model, `npvis` was also shown to be statistically significant, while `monpre` was either not significant or had a counterintuitive coefficient sign, so in this section we will focus on investigating `npvis`.
...
The numbers all come out to very near zero. This means that we can assume $\frac{\partial (bwght)}{\partial(npvis)} = \beta_{npvis}$, since $\frac{\partial u}{\partial(npvis)} = 0$. Therefore, we can say that npvis does have a positive causal effect on birthweight (all `npvis` coefficients are positive).

### Direction of biases

We believe that there are other factors that can impact the weight of a newborn baby. These may include the mother's and father's health condition (e.g. diet or presence of specific conditions such as gestational diabetes), and the mother's quality of life during pregnancy (e.g. stress level), income, among others. Therefore, none of the above three models above accurately refects the causality of `npvis` and `monpre` on `bwght`.

However, we still can draw some conclusions about causality by making some additional assumptions. We will use model1 in this analysis.

First, let's assume the only missing independent variable from model 1 compared to a 'true' population model is the mother's income (`in`). In model 1, from earlier analysis, `npvis` is the only significant variable so we can ignore `monpre`. 
There have been researches that pointed out low income can negatively impact infants's birth weight. 
For exampnle: https://www.guttmacher.org/journals/psrh/2007/infants-low-birth-weight-linked-low-income-mothers-chronic-stress

We can safely assume that `in` has a positive effect on the baby's weight -> $\beta_{in} > 0$.


We also assume that the better income the mother has, the more the number of prenatal visits she makes during preganancy, so the coeffient of `npvis` on `hf` is positive ($\beta_{npvis:in} > 0$).
This can be crossed check from relavant researches in the field. This link (http://www.apa.org/pi/ses/resources/publications/women.aspx)
cites this research: Lower income pregnant women receive less prenatal care, experience higher levels of stress and are more likely to deliver premature babies (Adler, 2006). 

So the bias is Beta(in)*Beta(npvis:in) > 0. In other words, the current regression in model 1 for npvis seems to overestimate the coefficient for npvis.
...
coeftest(model4)
```
This shows that monpre is indeed absorbing some of the causal effect from npvis.
...
+ *Highly correlated parents:* The mother's and father's demographics were highly correlated. Including both in the model caused distortions in our results, so we believe it is best to include only the mother's demographics.

+ *Omitted variables:* Our low $R^2$ values show that we are missing some key variables that would help explain the variation in `bwght`. Examples include the mother's income, diet, stress level, and quality of care received, among others.
